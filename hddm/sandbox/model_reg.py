...
class wfpt_regress_gen(stats.distributions.rv_continuous):
    ...
    def _logp(self, x, v, V, a, z, Z, t, T, reg_params):
        """Log-likelihood for the full DDM using the interpolation method"""
        return hddm.wfpt.wiener_like_multi(value, v, V, a, z, Z, t, T, .001, reg_params)

    def _pdf(self, x, v, V, a, z, Z, t, T, reg_params):
        raise NotImplementedError
    
    def _rvs(self, v, V, a, z, Z, t, T, reg_params):
    ...
    def random(self, v=1., V=0., a=2, z=.5, Z=.1, t=.3, T=.1, reg_params=None, size=None):
        print "in random"
        self._size = len(locals()[reg_params[0]])
        return self._rvs(v, V, a, z, Z, t, T, reg_params)
...
class HDDMRegressor(hddm.model.HDDM):
    def __init__(self, data, regressor=None, **kwargs):
        ...
        self.reg_outcome = [] # holds all the parameters that are going to modeled as outcome
        for reg in regressor:
            if type(reg['args']) == str:
                reg['args'] = [reg['args']]
            if type(reg['covariates']) == str:
                reg['covariates'] = [reg['covariates']]
            self.reg_outcome.append(reg.outcome)
    ...
    def create_params(self):
        ...
        remove_params = ['wfpt'] + self.reg_outcome
        ...
        for i_reg, reg in enumerate(self.regressor):
            for arg in reg['args']:
                ...
                reg_g = Knode(pm.Normal, mean=0, tau=10**-2, value=0)
    ...
    def get_bottom_node(self, param, params):
        ...
        for reg in self.regressors:
            ...
            try:
                params[outcome] = func(reg['args'], predictors)
            except TypeError:
                errmsg = """the function of %s raised an error. make sure that the first argument is a list of
                of arguments, and the second is a numpy array or matrix"""

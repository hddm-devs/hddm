...
class NetworkInspectorTest(unittest.TestCase):
    ...
    def test_load_torch_model_and_run_prediction(self):        
        for model in self.models:
            ...
            rts = np.expand_dims(np.concatenate([np.arange(self.rt_range[0], self.rt_range[1], self.rt_steps) for i in range(n_choices)]), axis = 1)
            choices = np.concatenate([[c for i in range(int((self.rt_range[1] - self.rt_range[0]) / self.rt_steps))] for c in choice_options])

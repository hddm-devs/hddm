...
class ModelLoad(unittest.TestCase):
    def get_data_single_subj(self, model = None):
        data, gt_parms__ = hddm.simulators.hddm_dataset_generators.simulator_single_subject(
    ...
    def setUp(self):
        self.models = list(hddm.torch.torch_config.TorchConfig(model = 'ddm').network_files.keys())
    ...
    def test_init_sample_save_load_stimcoding(self):
        for model in self.models:
            if hddm.model_config.model_config[model]['n_choices'] == 2:
                # Generate Data
                data, gt = hddm.simulators.hddm_dataset_generators.simulator_stimcoding(model = model,
                                                                             n_samples_by_condition= self.n_samples_per_trial,
                                                                             split_by = 'v')
                # Initialize HDDM Model 
                model_ = hddm.HDDMnnStimCoding(data,
                                               model = model,
                                               network_type = 'torch_mlp',
                                               split_param = 'v',
    ...
    def test_init_sample_save_load_regressor(self):
        for model in self.models:
            if hddm.model_config.model_config[model]['n_choices'] == 2:
                ...
                model_ = hddm.HDDMnnRegressor(self.cav_data, 
                                             [v_reg],
                                             include = hddm.model_config.model_config[model]['hddm_include'],
                                             model = model,
                ...
                self.assertTrue(model__.model_descrs[0]['model']['link_func'] == id_link)

...
# ):
#     all_data = []
#     print("using sim param values of -- v = %f, a = %f, z = %f, t = %f" % (scaler, a, z, t))
#     print("alpha = ", alpha)
...
#         print("sim-vals [0] = ", np.array([df.loc[0, "sim_drift"], a, z, t]))
#         # simulate model with given params
#         res = simulator(
#             np.array([df.loc[0, "sim_drift"], a, z, t]),
#             model='ddm',
#             n_samples=1,
...
#             print("sim-vals [i] = ", np.array([df.loc[i, "sim_drift"], a, z, t]))
#             # simulate model with given params
#             res = simulator(
#                 np.array([df.loc[i, "sim_drift"], a, z, t]),
#                 model='ddm',
#                 n_samples=1,
#                 delta_t=0.001,  # n_trials
#                 max_t=20,
...
def gen_rand_rlssm_data_MAB_qlearn(
    ...
    # print("using sim param values of -- v = %f, a = %f, z = %f, t = %f" % (ssm_params[0], ssm_params[1], ssm_params[2], ssm_params[3]))
    # print("alpha = ", rl_params[0])

    for s in range(0, subjs):
        scaler = ssm_params[0]
        alpha = rl_params[0]
        if dual == True:
            pos_alfa = rl_params[1]
        else:
            pos_alfa = alpha
        ...
        sim_params = np.append([df.loc[0, "sim_drift"]], ssm_params[1:])

        #print("sim-vals [0] = ", sim_params)
        # simulate model with given params
        res = simulator(
            sim_params,
            model=model,
        ...
        for i in range(1, n):
            ...
            # Get sim_params for passing into the simulator
            sim_params = np.append([df.loc[i, "sim_drift"]], ssm_params[1:])
            #print("sim-vals [i] = ", sim_params)

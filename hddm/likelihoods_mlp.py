...
def make_mlp_likelihood(model, **kwargs):
    ...
    wfpt_nn = stochastic_from_dist(
        "Wienernn_" + model, partial(likelihood_, **kwargs)
    )
...
def make_mlp_likelihood_reg(model=None, **kwargs):
    ...
    # New way of specifying likelihoods for included models
    # (eventually use only this for conciseness)

    likelihood_ = make_likelihood(model = model)
    # likelihood_funs[model] = likelihood_
    ...
    # Keeping below function as example

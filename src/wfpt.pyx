...
def wiener_like_rlssm_nn_pdf(str model, 
                      np.ndarray[double, ndim=1] x,
                      np.ndarray[long, ndim=1] response,
                      np.ndarray[double, ndim=1] feedback,
                      np.ndarray[long, ndim=1] split_by,
    ...
    if p_outlier == 0: # ddm_model
        sum_logp = np.squeeze(np.core.umath.maximum(network.predict_on_batch(data), ll_min))
    else: # ddm_model
        sum_logp = np.squeeze(np.log(np.exp(np.core.umath.maximum(network.predict_on_batch(data), ll_min)) * (1.0 - p_outlier) + (w_outlier * p_outlier)))
    if sum_logp == 0:
        sum_logp = np.exp(sum_logp) # shouldn't be called log_p anymore but no need for an extra array here

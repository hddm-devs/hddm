...
def wiener_like_rlddm(np.ndarray[double, ndim=1] x, 
    ...
    cdef double both = np.unique(split_by, return_inverse=True)
    cdef double u = both[0]
    cdef double indices = both[1]
    ...
    for s in range(u.shape[0]):
        ...
        for i in range(1,size):
            ...
            exp_lows[i] = (exp_lows[i-1]*(responses[i-1])) + ((1-responses[i-1])*(exp_lows[i-1]+(alfalfa*(rew_lows[i-1]-exp_lows[i-1]))))
